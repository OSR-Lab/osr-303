...
int main(void)
{
  ...
  while (1)
  {
   ...
   ret = HAL_UART_Receive(&huart1, pt, 16, HAL_MAX_DELAY); //receive 16 bytes of plaintext from the serial port
   if(ret==HAL_OK)
   {
     ...
     HAL_UART_Transmit(&huart1, ct, 16, HAL_MAX_DELAY); //send 16 bytes of ciphertext to the serial port
	  }
    /* USER CODE END WHILE */
