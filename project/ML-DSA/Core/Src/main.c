...
// ML-DSA-44 Ref: https://github.com/mupq/pqm4/tree/master/crypto_s/ml-dsa-44/m4fstack
#include "api.h"
#define MLEN 16
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if(ret==HAL_OK)
	  {
		  ...
		  //HAL_UART_Transmit(&huart1, pk, CRYPTO_PUBLICKEYBYTES, HAL_MAX_DELAY);
		  HAL_UART_Transmit(&huart1, sig, CRYPTO_BYTES, HAL_MAX_DELAY);

		  __disable_irq();  // disable interrupts
		  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, 1); // drive the level high before the operation as a trigger signal
		  vry = crypto_sign_verify(sig, CRYPTO_BYTES, m, MLEN, pk);
		  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, 0); // drive the level low after the operation
		  __enable_irq();  // enable interrupts
