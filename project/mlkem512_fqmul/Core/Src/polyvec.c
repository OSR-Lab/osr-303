...
#include <stdint.h>
#include "ntt.h"
#include "main.h"  // Reason: used for PA6 trigger control
/*************************************************
...
void PQCLEAN_MLKEM512_CLEAN_polyvec_basemul_acc_montgomery(poly *r, const polyvec *a, const polyvec *b) {
    ...
    // Reason: the PA6 trigger precisely covers the basemul operation of the first polynomial, for SCA acquisition
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);
    PQCLEAN_MLKEM512_CLEAN_poly_basemul_montgomery(r, &a->vec[0], &b->vec[0]);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);

    for (i = 1; i < KYBER_K; i++) {
//    	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);  //attack the second polynomial
