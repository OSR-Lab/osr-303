...
uint32_t random_number(void) {
    ...
    seed = (A * seed + C) % M;  // Generate next random number
    return seed;
}


void p256_generate_random(uint8_t * output, unsigned output_size) {
	for (unsigned i = 0; i < output_size; i++) {
	        // Get a random number and extract a byte from it
	        uint32_t rand_val = random_number();
	        output[i] = (uint8_t)(rand_val & 0xFF);  // Take the lowest byte
	    }
}
                               ...
                               uint32_t x[8], uint32_t y[8])
{
    ...
    do {
        ...
        p256_generate_random(sbytes, 32);
        CT_POISON(sbytes, 32);
