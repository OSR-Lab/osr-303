...
// Finalize the SHA-256 hash and produce the digest
...
void hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *data, size_t data_len, uint8_t *hmac) {
    uint8_t key_block[SHA256_BLOCK_SIZE];
    uint8_t o_key_pad[SHA256_BLOCK_SIZE], i_key_pad[SHA256_BLOCK_SIZE];
